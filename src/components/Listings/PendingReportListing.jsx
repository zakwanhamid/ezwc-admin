...
import ModalIgnoreListing from './ModalIgnoreListing';

export default function PendingReportListing({reports, onReportUpdate}) {
  ...
  return (
    <div className="overflow-x-auto bg-white rounded-lg shadow-lg p-6 mt-4 w-full">
          ...
          {filteredReports
            ...
            .map((report, index) => (
                ...
                <tr className="cursor-pointer" onClick={() => handleRowClick(index)}>
                  ...
                  <td className="py-2 px-4 border-b">{report.listingId}</td>
                  <td className="py-2 px-4 border-b text-center">{report.sellerName}</td>
                  <td className="py-2 px-4 border-b text-center">{report.reporterName}</td>
                  <td className="py-2 px-4 border-b text-center">{report.listingCategory}</td>
                    ...
                    <td colSpan="6" className="p-4">
                      <div className='relative'>
                        <div className='flex flex-row'>
                            <div className='mr-12'>
                                <div><strong>Seller name:</strong> {report.sellerName}</div>
                                <div><strong>Seller Email:</strong> {report.sellerEmail}</div>
                            </div>
                            <div>
                        ...
                        <div>
                            <div><strong>Listing time:</strong> {report.listingTime.toDate().toLocaleString('en-US', options)}</div>
                            <div><strong>Listing title:</strong> {report.listingTitle}</div>
                            <div><strong>Listing Price:</strong> RM {report.listingPrice}</div>
                            <div><strong>Listing description:</strong> {report.listingDesc}</div>
                            <div className='flex flex-row'>
                            <div><strong>Listing image:</strong> </div>
                            {report.listingImage && report.listingImage.length > 0 ?
                                <img src={report.listingImage} alt="" className="w-52 h-32 rounded m-2" /> :
      ...
      {showModalDelete && <ModalDeleteListing
        onClose={() => setShowModalDelete(false)} 
        reportId={currentReport.id} 
        listingId={currentReport.listingId}
        onReportUpdate={onReportUpdate} />}
      {showModalIgnore && <ModalIgnoreListing
